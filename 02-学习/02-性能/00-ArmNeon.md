...
![[Pasted image 20230526173007.png]]

每个128-bit向量寄存器可以当做：
-  2 个 64-bit 元素的向量寄存器 -- vn.2d；
-  4 个 32-bit 元素的向量寄存器 -- vn.4s；
-  8 个 16-bit 元素的向量寄存器 -- vn.8h；
-  16 个 8-bit 元素的向量寄存器 -- vn.16b；
每个向量寄存器也可以只用低 64-bit：
-  1 个 64-bit 元素的向量寄存器 -- vn.1d；
-  2 个 32-bit 元素的向量寄存器 -- vn.2s；
-  4 个 16-bit 元素的向量寄存器 -- vn.4h；
-  8 个 8-bit 元素的向量寄存器   -- vn.8b；
...
LD1 { V0.4S, V1.4S }, [X0]
//将[X0]处的8个单精度浮点数按顺序加载到第一个V0，然后是V1的各个通道中。这也可以与寄存器和立即偏移一起使用。
LD1 { V0.S }[0], [X0]
//将位于[X0]的一个单精度浮点数加载到V0寄存器的四个通道中的第一个通道，即4S或V0.4S[0]。
LD1R { V0.S }, [X0]
//将位于[X0]的一个单精度浮点数加载到V0寄存器的四个通道中，作为4S
LD2 { V0.S, V1.S }, [X0]
//将[X0]处的交错数列加载到两个SIMD寄存器V0和V1中，使得[X0]处的第一个值进入V0的第一条通道，第二个值进入V1的第一条通道，以此类推。
LD2 { V0.S, V1.S }[0], [X0]  
...
float32x4_t v_reciprocal = vrecpsq_f32(v_input, vdupq_n_f32(1.0f));*/
...
/*--1、Vector shift left(饱和指令): vshl -> ri = ai << bi; 
负值向右移位
左移将向量中的每个元素按照第二个输入向量中指定的数量进行移位。
移位量是第二个输入向量元素最低有效字节的有符号整数值。 移出每个元素的比特将丢失。
如果有符号整数值为负，则会导致右移。--*/
...
如果移位值为正，则操作是左移。否则，它是一个截断的右移。
左移每个整数向量中的元素，并将结果放置在目标向量中。
这类似于VSHL。不同之处在于，如果发生饱和，则设置粘性QC标志。
*/  
...
如果移位值为正，则操作是左移。否则，它是一个舍入右移。
左移将整数向量中的每个元素都向左移动，并将结果放置在目标向量中。这类似于VSHL。
不同之处在于，然后会对移位值进行四舍五入 --*/  
...
左移位每个整数向量元素并将结果放入目标向量中。它类似于VSHL。
不同之处在于，移位值被舍入，并且如果发生饱和，则设置粘性QC标志。 --*/  
...
结果被截断。right shifts 每个向量元素一个const值，并将结果累加到目标向量中。 --*/  
